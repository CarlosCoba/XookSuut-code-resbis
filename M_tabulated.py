...
def M_tab(pa,inc,x0,y0,theta_b,rings, delta,k, shape, vel_map, evel_map, pixel_scale=1,vmode = "radial"):
	...
	if vmode == "resbis":
		...
		sigma_v = e_vel[mask]
		x22,x23 = w_rad**2/sigma_v**2,w_rad*w_tan/sigma_v**2
		x32,x33 = w_rad*w_tan/sigma_v**2,w_tan**2/sigma_v**2
		...
		A = np.asarray([[np.nansum(x22),np.nansum(x23)],[np.nansum(x32),np.nansum(x33)]])
		B= np.asarray([np.nansum(y2),np.nansum(y3)])


		try:
			x = np.linalg.solve(A, B)
			vrot,vrad,vtan = 0,x[0],x[1]
			if np.isfinite(vrot) == False: vrot = 50
			if np.isfinite(vrad) == False: vrad = 0
			if np.isfinite(vtan) == False: vtan = 0
