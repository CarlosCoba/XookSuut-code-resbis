...
def bayesian_mcmc(galaxy, shape, vel_map, e_vel_map, guess, vmode, config, rings_pos, ring_space, steps, e_ISM = 5, pixel_scale = 1, r_bar_min = None, r_bar_max = None):
	...
	vrot0 = np.zeros(len(vrot0),)

	vel_map[abs(vel_map) > 500] = np.nan
	n_circ = len(vrot0)
	non_zero = vrad0 != 0
	...
	def model(theta):
		...
		def eval_mdl(xy_mesh,pa,inc,x0,y0,Vsys,phi_b, r_0 = None, r_space = ring_space,ii=0):
			...
			Vrot_ = 0#Vrot_dic["Vrot%s"%ii]
			if ii + 1 <= n_noncirc:
				Vrad_ = Vrad_dic["Vrad%s"%ii]
				Vtan_ = Vtan_dic["Vtan%s"%ii]
				bisymm = BISYM_MODEL(xy_mesh,Vrot_,Vrad_,pa0,inc0,X0,Y0,Vtan_,phi_b)
			else:
				bisymm = BISYM_MODEL(xy_mesh,Vrot_,0,pa0,inc0,X0,Y0,0,0)

			W = weigths_w(xy_mesh,shape,pa0,inc0,X0,Y0,r_0,r_space,pixel_scale=pixel_scale)
			w0,w1 = weigths_w(xy_mesh,shape,pa0,inc0,X0,Y0,r_0,r_space,pixel_scale=pixel_scale) 
			modl0 = bisymm*w0
			modl1 = bisymm*w1
		...
		for N in range(n_annulus):
			...
			for kk in range(2):
				Vxy,Vsys = eval_mdl(XY, pa0,inc0,X0,Y0,Vsys,phi_b, r_0 = rings_pos[N], r_space = r_space_k, ii = N+kk)
				...
				if N == 0 and kk == 0:
					...
					Vxy,Vsys  = eval_mdl(XY1, pa0,inc0,X0,Y0,Vsys,phi_b, r_0 = 0, r_space = r_space_0, ii = 0)
	...
	def ln_likelihood(theta):
		...
		MODEL, OBS, ERROR = model_array, obs_array, error_array
		N = len(OBS)
	...
	def ln_prior(theta):
		...
		#if  0 <= pa <= 360 and 0 < inc < 90 and 0 < x0 < nx and 0 < y0 < ny and -100 < Vsys < 100 and 0 < phi_b < 180:
		if -100 < Vsys < 100 and 0 < phi_b < 180:
	...
	theta0 = np.hstack(theta_array.flatten())
	ndim = len(theta0)
	...
	axes = fig_ambient(fig_size = (2,1.3), ncol = 1, nrow = 1, left = 0.12, right = 0.99, top = 0.99, hspace = 0, bottom = 0.15, wspace = 0.4 )
	ax = axes[0]
	XX = np.arange(steps)
	...
	plt.savefig("./plots/accept_rate.%s_model.%s.pdf"%(vmode,galaxy))
	plt.tight_layout()
	plt.clf()
	...
	vlos_2D_model = best_2d_model(vmode, shape,V_k, pa0, inc0, X0, Y0, Vsys, rings_pos, ring_space = ring_space, phi_b = phi_b, pixel_scale = pixel_scale) 

	##########
	from create_2D_kin_models import bidi_models
	kin_2D_models = bidi_models(vmode, shape, V_k, pa0, inc0, X0, Y0, Vsys, rings_pos, ring_space = ring_space, pixel_scale = pixel_scale) 
	##########
	
	epa,einc,ex0,ey0 = 0, 0, 0, 0
	eVrot = eVrot*0
	Std_errors = eVrot,eVrad,epa,einc,ex0,ey0,eVsys,ephi_b,eVtan


	return chain,vlos_2D_model, kin_2D_models, vrot0, Vrad, Vsys,  pa0, inc0 , X0, Y0, Vtan, phi_b, Std_errors
