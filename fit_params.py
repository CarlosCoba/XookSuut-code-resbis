...
#####
	...
	if vmode == "resbis":
		...
		vel_map[abs(vel_map) > 200] = np.nan
		def vmodel_dataset(pars, i,  xy_mesh,  r_0 = None, r_space = ring_space):
			...
			if Vrad != 0 and Vtan != 0:
				...
				modl = (BISYM_MODEL(xy_mesh,0,Vrad,pa,inc,x0,y0,Vtan,phi_b))*weigths_w(xy_mesh,shape,pa,inc,x0,y0,r_0,r_space,pixel_scale=pixel_scale)
			else:

				modl = (BISYM_MODEL(xy_mesh,0,0,pa,inc,x0,y0,0,0))*weigths_w(xy_mesh,shape,pa,inc,x0,y0,r_0,r_space,pixel_scale=pixel_scale)
		...
		for iy in range(nrings):
				...
				fit_params.add('Vrad_%i' % (iy), value=vr20[iy], vary = vary_vrad,  min = -400, max = 400)
				fit_params.add('Vtan_%i' % (iy), value=vt20[iy], vary = vary_vtan, min = -400, max = 400)
