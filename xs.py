...
from bisymetric_mode_res import res_mod
from bisymetric_mode_twostep import twostep_mod
...
def init_models(galaxy,vel,evel,guess0,vary,n_it,sigma, delta,  rstart, rfinal, ring_space, frac_pixel, bar_min_max, pixel_scale, vmode, errors, config, e_ISM, steps, method , use_metropolis):
	...
	if vmode == "resbis":
		PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,best_vlos_2D_mdl, best_kin_2D_mdls,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r,res_MCMC  = res_mod(galaxy, vel, evel, guess0, vary, n_it, rstart =  rstart, rfinal = rfinal, ring_space = ring_space, frac_pixel = frac_pixel, delta = delta, pixel_scale = pixel_scale, bar_min_max = bar_min_max, errors = errors,  config = config, e_ISM = e_ISM, steps = steps, method = method, use_metropolis = use_metropolis)

		return PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,best_vlos_2D_mdl, best_kin_2D_mdls,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r,res_MCMC


	if vmode == "twostep":
		PA,INC,XC,YC,VSYS,THETA,R,Vrot,Vrad,Vtan,best_vlos_2D_mdl, best_kin_2D_mdls,PA_BAR_MAJOR,PA_BAR_MINOR,CHISQ_r,res_MCMC  = twostep_mod(galaxy, vel, evel, guess0, vary, n_it, rstart =  rstart, rfinal = rfinal, ring_space = ring_space, frac_pixel = frac_pixel, delta = delta, pixel_scale = pixel_scale, bar_min_max = bar_min_max, errors = errors,  config = config, e_ISM = e_ISM, steps = steps, method = method, use_metropolis = use_metropolis)
