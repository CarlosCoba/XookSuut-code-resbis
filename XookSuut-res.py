...
model	= 	The different kinematic models. You must choose between: "circular" motion (circular), "radial" flows (radial), or "bisymmetric" (oval distortion).
...
errors	=	Boolean. If you want to compute errors for the derived parameters using a Metropolis-Hastings analysis. Otherside it will use the errors computed by the
		chisquare analysis.
...
if __name__ == "__main__":
	...
	if (nargs == 34 ):
		...
		print ("USE: XookSuut-res.py object vel_map [evel_map] SN [VSYS] PA INC X0 Y0 [N_it=5] [pixel_scale=1] vary_PA[0,1] vary_INC[0,1] vary_XC[0,1] vary_YC[0,1] vary_VSYS[0,1] vary_PHI_bar[0,1] ring_space [Delta] [Rstart=2.5] [Rfinal=40] [frac_pixel=2/3.] [R_bar_min,R_bar_max] [model=cicular,radial,bisymmetric] [save_plots=1] [errors=0] [survey] [config_file] [e_ISM=5] [steps = 1e4] [method = Powell, leastsq] [use_mh = 0] [prefix]" )
