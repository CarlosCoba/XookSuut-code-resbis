...
def twostep_mod(galaxy, vel, evel, guess0, vary, n_it, rstart, rfinal, ring_space, frac_pixel, delta, pixel_scale,bar_min_max,errors, config, e_ISM, steps, method, use_metropolis):
		...
		for it in np.arange(n_it):
			...
			vrot_tab, vrad_tab0, vtan_tab0, R_pos = tab_mod_vels(rings,vel, evel, pa0,inc0,x0,y0,vsys0,theta_b0,delta,pixel_scale,"circular",shape,frac_pixel,r_bar_min, r_bar_max)
			vrot_tab0, vrad_tab, vtan_tab, R_pos = tab_mod_vels(rings,vel, evel, pa0,inc0,x0,y0,vsys0,theta_b0,delta,pixel_scale,"bisymmetric",shape,frac_pixel,r_bar_min, r_bar_max)

			vrot_tab[abs(vrot_tab) > 400] = np.nanmedian(vrot_tab)
			guess = [vrot_tab,vrad_tab,pa0,inc0,x0,y0,vsys0, vtan_tab,theta_b0]

			v_2D_mdl,  kin_2D_modls,  vrot , vrad, vsys0,  pa0, inc0, x0, y0, vtan,theta_b, xi_sq, n_data, Errors = fit(shape, vel, evel, guess, vary, "bisymmetric", config, R_pos, fit_method = method, e_ISM = e_ISM, pixel_scale = pixel_scale, ring_space = ring_space )
		...
		if errors == 1:
			...
			chain,v_2D_mdl_, kin_2D_models_, Vrot_, Vrad_, Vsys_,  PA_, INC_ , XC_, YC_, Vtan_, THETA_, std_errors = bayesian_mcmc(galaxy, shape, vel, evel, GUESS, "twostep", config, Rings, e_ISM = e_ISM, pixel_scale = pixel_scale, ring_space = ring_space, r_bar_min = r_bar_min, r_bar_max = r_bar_max, steps = steps)
